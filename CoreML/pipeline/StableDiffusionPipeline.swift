...
public struct StableDiffusionPipeline: ResourceManaging {
    ...
    public init(textEncoder: TextEncoder, unet: Unet, decoder: Decoder, safetyChecker: SafetyChecker? = nil, reduceMemory: Bool = false) {
    ...
    ///   - seed: Random seed which allows us to re-generate the same image for the same prompt by re-using the seed. If the seed is -1, then a random seed is generated and returned at the end.
    ///   - guidanceScale: For classifier guidance
    ///   - disableSafety: Safety checks are only performed if `self.canSafetyCheck && !disableSafety`
    ///   - progressHandler: Callback to perform after each step, stops on receiving false response
    /// - Returns: A tuple containing an array of `imageCount` optional images and an `Int` for the random seed used. The images will be nil if safety checks were performed and found the result to be un-safe
    public func generateImages(prompt: String, negativePrompt: String = "", imageCount: Int = 1, stepCount: Int = 50, seed: Int = -1, guidanceScale: Float = 7.5, disableSafety: Bool = false, scheduler: StableDiffusionScheduler = .pndm, progressHandler: (Progress) -> Bool = { _ in true }) throws -> ([CGImage?], Int) {
		// Should we generate a random seed?
		let iseed = seed == -1 ? Int.random(in: 0...Int.max) : seed
...
//		let blankEmbedding = try textEncoder.encode("")
		...
		let concatEmbedding = MLShapedArray<Float32>(concatenating: [negativePromptEmbedding, promptEmbedding], alongAxis: 0)
        ...
        var latents = generateLatentSamples(imageCount, stdev: stdev, seed: iseed)
        // De-noising loop
        for (step,t) in scheduler[0].timeSteps.enumerated() {
            // Expand the latents for classifier-free guidance and input to the Unet noise prediction model
            let latentUnetInput = latents.map {
                MLShapedArray<Float32>(concatenating: [$0, $0], alongAxis: 0)
            }
            // Predict noise residuals from latent samples and current time step conditioned on hidden states
            var noise = try unet.predictNoise(latents: latentUnetInput, timeStep: t, hiddenStates: hiddenStates)
			noise = performGuidance(noise: noise, guidance: guidanceScale)
            // Have the scheduler compute the previous (t-1) latent sample given the predicted noise and current sample
            for i in 0..<imageCount {
                latents[i] = scheduler[i].step(output: noise[i], timeStep: t, sample: latents[i])
            }
            // Report progress
            let progress = Progress(pipeline: self, prompt: prompt, step: step, stepCount: stepCount, currentLatentSamples: latents, isSafetyEnabled: canSafetyCheck && !disableSafety)
		...
		let imgs = try decodeToImages(latents, disableSafety: disableSafety)
        return (imgs, iseed)
    }
